...
def delete_specific_city(city_id):
    """Deletes a specific City object if a correct city ID is given,
    404 error if not."""
    specific_city = storage.get(City, city_id)
    if specific_city is None:
        abort(404)
...
def create_city(state_id):
    """Creates a new city visa post using a JSON input,
    404 error if state_id is not linked to a specific state,
    400 error if not json dictionary or if dictionary does not contain name"""
